...
function 결과체크(몇줄, 몇칸) {  // 스코프 넘기 위해 매개변수로 넣은 것
  // 세칸 다 채워졌나?
  var 다참 = false;
  // 가로줄 검사
  if (
  ...
  return 다참; //괄호 마지막에 이렇게 하면 스코프 넘을 수 있음
}
...
var 비동기콜백 = function(이벤트) {
  ...
  if (칸들[몇줄][몇칸].textContent !== '') { // 칸이 이미 채워져 있는가?
    ...
    // filter는 true만 보여줌, false를 찾을거니까 앞에 !

    if (승리여부) { // 이겼을 경우
      초기화();     // === 초기화(undefiend), unde는 if 안에선 false임
...
for (var i = 1; i <= 3; i += 1) {
  ...
  for (var j = 1; j <= 3; j += 1) {
    ...
    칸.addEventListener('click', 비동기콜백);  // 클릭했을 때
    칸들[i - 1].push(칸); // 칸들의 Array 하나마다 3개씩 'td'넣기
    줄.appendChild(칸);
  }
  테이블.appendChild(줄);
}
